/*mono_cluster.cpp     June 2018 - January 2020 by Pia Backmann   */
...
int percol_cluster(int num_n, int N, int *next, int *site, int *cluster, World_t *welt, FILE *out, FILE *histogramm)
{
  ...
  //  fprintf(out, " %d\t%d\t%d\t%f\t%f\t%f\t%f\n",welt->tick, num_clusters, biggest_size,(double) biggest_size/N, (double) size_all/N,(double) biggest_nonperko/N, welt->mortality_rate);  // clusterstats
    if(welt->tick == 10000)
    {
         ...
         printf("%d\t%d\t%d\t%f\t%d \t %d \t%d\t%f\t%f\t%f\t%f\n", welt->size, welt->seed_mass, welt->seed, welt->mortality_rate, welt->init_option, num_clusters, biggest_size,(double) biggest_size/(double) N, (double) size_all/(double) N, (double) biggest_nonperko/(double)N, gyration_nonperko);
    }
    if(welt->tick == 1)
    {
	 char* myfileroot="";
