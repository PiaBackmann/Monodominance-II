...
typedef struct
{
    ...
    double shadetolerance;        // Describes how shade-tolerant different species are supposed to be (between 0 and 1 -> 1 = max tolerant)
...
typedef struct
{
    ...
    double shade;                  // value in [0 (no shade ... 1.0 (total shading)]
...
typedef struct
{
    ...
    double shadetolerance_neutral;
    double shadetolerance_mono;
    ...
    int density_radius;
    int init_option;
    int self_replacement;
    int geometric_replacement;               // 0 = mono seeds are distributed randomly, 1 = geometrically (1 per grid cell)
    double shade;
    int JC;
    int JCradius;
    int JC_maxcount;
