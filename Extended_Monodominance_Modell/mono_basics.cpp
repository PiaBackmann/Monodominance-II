/*mono_basics.cpp     June 2018 - January 2020 by Pia Backmann   */
...
/***************************************************/
{
    ...
    sprintf(name_outfile, "%sWeltview/Monodominanz_shade%s_seed_%d_tick_%d.out", welt->pfad, welt->ende, welt->seed, welt->tick);
    out = fopen(name_outfile, "w");
    for(int i = 0; i < welt->size; i++)
    {
        for(int j = 0; j < welt->size; j++)
        {
            fprintf(out,"%1.3f ", welt->grid[i][j]->shade) ;
        }
         fprintf(out,"\n") ;
    }
    fclose(out);
